...
class Reserve_Controller_Handler:
    ...
    * CRUD OPERATIONS
    ...
    def Post_Reserve(self, reserve_data):
        ...
        room = daoroom.Get_Room_info(rid)
        hid = room[1]
        ...
        if startdate <= availability[0]:
            txt = "Room unavailable during selected dates. Start date: " + str(startdate) + ", End date: " + str(enddate) + ", Availability: " + str(availability[0])
        ...
        if payment is not None:
            ...
            if not ruid:
                return jsonify(Error="Unexpected attribute value")
